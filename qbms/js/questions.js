...
function updateQuestionCount() {
  const numQuestionsInput = document.getElementById('quiz-num-questions');
  if (numQuestionsInput) {
    numQuestionsInput.value = selectedQuestionIds.length;
    calculateTotalMarks();
  }
}

function calculateTotalMarks() {
  const numQuestions = parseInt(document.getElementById('quiz-num-questions').value) || 0;
  const pointsPerQuestion = parseInt(document.getElementById('quiz-points-per-question').value) || 0;
  const totalMarksInput = document.getElementById('quiz-total-marks');
  if (totalMarksInput) {
    totalMarksInput.value = numQuestions * pointsPerQuestion;
  }
}
...
document.getElementById('create-quiz-btn').addEventListener('click', () => {
  ...
  updateQuestionCount(); // Update question count when modal opens
  calculateTotalMarks(); // Calculate total marks when modal opens
});

document.getElementById('quiz-points-per-question').addEventListener('input', calculateTotalMarks);

document.getElementById('closeCreateQuizModal').addEventListener('click', () => {
...
document.getElementById('createQuizForm').addEventListener('submit', async e => {
  ...
  const points_per_question = document.getElementById('quiz-points-per-question').value;
  const duration = document.getElementById('quiz-duration').value;

  try {
    // Fetch selected questions and their options
    const { data: questions, error: questionsError } = await supabase
      .from('questions')
      .select('id, question_text, options (option_text, is_correct)')
    ...
    const quizData = {
      formTitle: title,
      formDescription: 'Answer the following questions.',
      questions: questions.map(q => ({
        questionText: q.question_text,
        options: q.options.map(o => o.option_text),
        correctAnswer: q.options.find(o => o.is_correct).option_text,
        isRequired: true, // Assuming all questions are required
        points: parseInt(points_per_question)
      }))
    };

    // Send the JSON data to the server
    // NOTE: This is a workaround for CORS issues. The correct solution is to configure the Google Apps Script to handle CORS requests.
    const url = 'http://localhost:8080/api/questions/google-form';
    ...
    const { data: exam, error: examError } = await supabase
      ...
      .insert({
        ...
        total_marks: selectedQuestionIds.length * parseInt(points_per_question),
        duration,
        editor_link: formResponse.formUrl, // Assuming the response contains the formUrl
        respondent_link: formResponse.formUrl.replace('/edit', '/viewform') // Assuming the response contains the formUrl
      })
      .select()
      .single();
    ...
    // Insert into exam_sections table
    ...
    const examQuestions = selectedQuestionIds.map(question_id => ({
    ...
    showSuccessModal('Quiz created and stored successfully!');
    document.getElementById('createQuizModal').style.display = 'none';
    selectedQuestionIds = [];
    updateQuestionCount();
    loadQuestions();
